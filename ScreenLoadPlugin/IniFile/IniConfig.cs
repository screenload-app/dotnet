		...
		private static string CreateIniLocation(string configFilename, bool isReadOnly) {
			...
			try {
				if (IniDirectory != null && Directory.Exists(IniDirectory)) {
					// If the ScreenLoad.ini is requested, use the supplied directory even if empty
					if (!isReadOnly) {
			...
			using (var memoryStream = new MemoryStream()) {
				using (TextWriter writer = new StreamWriter(memoryStream, Encoding.UTF8)) {
					...
					foreach (string sectionName in _sections.Keys) {
						...
						writer.WriteLine("; The section {0} hasn't been 'claimed' since the last start of ScreenLoad, therefor it doesn't have additional information here!", sectionName);
