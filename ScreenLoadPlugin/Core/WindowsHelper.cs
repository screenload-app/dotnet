...
namespace ScreenLoadPlugin.Core {
	...
	/// but a LOT of changes/enhancements were made to adapt it for ScreenLoad.
	///
	...
	public class WindowDetails : IEquatable<WindowDetails>{
		...
		private bool CanFreezeOrUnfreeze(string titleOrProcessname) {
			...
			if (titleOrProcessname.ToLower().Contains("ScreenLoad")) {
				return false;
			}
		...
		/// <summary>
		/// Check if this window is ScreenLoad
		/// </summary>
		public bool IsScreenLoad {
			get {
				try {
					if (!IsMetroApp) {
						using (Process thisWindowProcess = Process) {
							return "ScreenLoad".Equals(thisWindowProcess.MainModule.FileVersionInfo.ProductName);
						}
					}
				} catch (Exception ex) {
					Log.Warn(ex);
