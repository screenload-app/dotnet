...
namespace ScreenLoadPlugin.Core {
	...
	public static class ImageHelper {
		...
		/// <summary>
		/// This is a factory method to create a surface, set from the ScreenLoad main project
		/// </summary>
		public static Func<ISurface> SurfaceFactory { get; set; }

		static ImageHelper()
		{
			StreamConverters["ScreenLoad"] = (stream, s) =>
			{
		...
		/// <summary>
		/// Load a ScreenLoad surface from a stream
		/// </summary>
		/// <param name="surfaceFileStream">Stream</param>
		/// <param name="returnSurface"></param>
		/// <returns>ISurface</returns>
		public static ISurface LoadScreenLoadSurface(Stream surfaceFileStream, ISurface returnSurface)
		{
			Image fileImage;
			...
			{
				Log.DebugFormat("Loaded .ScreenLoad file with Size {0}x{1} and PixelFormat {2}", tmpImage.Width, tmpImage.Height, tmpImage.PixelFormat);
				fileImage = Clone(tmpImage);
			}
			// Start at -14 read "ScreenLoadXX.YY" (XX=Major, YY=Minor)
			const int markerSize = 14;
			surfaceFileStream.Seek(-markerSize, SeekOrigin.End);
			using (StreamReader streamReader = new StreamReader(surfaceFileStream))
			{
				var ScreenLoadMarker = streamReader.ReadToEnd();
				if (!ScreenLoadMarker.StartsWith("ScreenLoad"))
				{
					throw new ArgumentException("Stream is not a ScreenLoad file!");
				}
				Log.InfoFormat("ScreenLoad file format: {0}", ScreenLoadMarker);
			...
			if (fileImage != null)
			{
				...
				Log.InfoFormat("Information about .ScreenLoad file: {0}x{1}-{2} Resolution {3}x{4}", fileImage.Width, fileImage.Height, fileImage.PixelFormat, fileImage.HorizontalResolution, fileImage.VerticalResolution);
			}
