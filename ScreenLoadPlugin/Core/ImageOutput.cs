...
namespace ScreenLoadPlugin.Core {
	...
	public static class ImageOutput {
		...
		/// <param name="surface">surface for the elements, needed if the ScreenLoad format is used</param>
		/// <param name="stream">Stream to save to</param>
		/// <param name="outputSettings">SurfaceOutputSettings</param>
		public static void SaveToStream(Image imageToSave, ISurface surface, Stream stream, SurfaceOutputSettings outputSettings) {
			...
			if (outputSettings.Format == OutputFormat.ScreenLoad && surface == null) {
				throw new ArgumentException("Surface needs to be set when using OutputFormat.ScreenLoad");
			}
			...
			try {
			    ...
			    if (outputSettings.Format != OutputFormat.ScreenLoad)
			    {
			        return;
			    }
			    using (MemoryStream tmpStream = new MemoryStream()) {
			        ...
			        using (BinaryWriter writer = new BinaryWriter(tmpStream)) {
			            ...
			            byte[] marker = Encoding.ASCII.GetBytes($"ScreenLoad{v.Major:00}.{v.Minor:00}");
			            writer.Write(marker);
		...
		public static bool CreateImageFromSurface(ISurface surface, SurfaceOutputSettings outputSettings, out Image imageToSave) {
			...
			if (outputSettings.Format == OutputFormat.ScreenLoad || outputSettings.SaveBackgroundOnly) {
			...
			// The following block of modifications should be skipped when saving the ScreenLoad format, no effects or otherwise!
			if (outputSettings.Format == OutputFormat.ScreenLoad) {
				return disposeImage;
			}
			Image tmpImage;
		...
		/// <summary>
		/// Add the ScreenLoad property!
		/// </summary>
		/// <param name="imageToSave"></param>
		private static void AddTag(Image imageToSave) {
			// Create meta-data
			PropertyItem softwareUsedPropertyItem = CreatePropertyItem(PROPERTY_TAG_SOFTWARE_USED, "ScreenLoad");
		...
		/// <summary>
		/// Load a ScreenLoad surface
		/// </summary>
		/// <param name="fullPath"></param>
		/// <param name="returnSurface"></param>
		/// <returns></returns>
		public static ISurface LoadScreenLoadSurface(string fullPath, ISurface returnSurface) {
			...
			using (Stream surfaceFileStream = File.OpenRead(fullPath)) {
				returnSurface = ImageHelper.LoadScreenLoadSurface(surfaceFileStream, returnSurface);
			}
		...
		public static string SaveNamedTmpFile(ISurface surface, ICaptureDetails captureDetails, SurfaceOutputSettings outputSettings) {
			...
			if (string.IsNullOrEmpty(pattern?.Trim())) {
				pattern = "ScreenLoad ${capturetime}";
			}
