...
namespace ScreenLoadPlugin.Core {
	...
	public class LogHelper {
		...
		private const string InitMessage = "ScreenLoad initialization of log system failed";
		...
		public static string InitializeLog4Net() {
			...
			foreach (var logName in new[] { "log4net.xml" , @"App\ScreenLoad\log4net-portable.xml"})
			{
			...
			if (!_isLog4NetConfigured) {
				try {
					...
					using (Stream stream = assembly.GetManifestResourceStream("ScreenLoadPlugin.log4net-embedded.xml")) {
