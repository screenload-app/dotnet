...
namespace ScreenLoadPlugin.Core {
	...
	public enum OutputFormat {
		bmp, gif, jpg, png, tiff, ScreenLoad, ico
	}
	...
	/// </summary>
	[IniSection("Core", Description="ScreenLoad core configuration")]
	public class CoreConfiguration : IniSection, INotifyPropertyChanged
    {
        ...
        [IniProperty("OutputFileAllowOverwrite", Description = "If the target file already exists True will make ScreenLoad always overwrite and False will display a 'Save-As' dialog.", DefaultValue = "true")]
        ...
        [IniProperty("OpenFolderAfterImageSaved", DefaultValue = "false")]
        public bool OpenFolderAfterImageSaved { get; set; }

        [IniProperty("OutputFileAsFullpath", Description="SaveAs Full path?")]
		public string OutputFileAsFullpath { get; set; }

        [IniProperty("LastSavedUrl")]
        public string LastSavedUrl { get; set; }
        ...
        [IniProperty("NotificationSound", Description = "The wav-file to play when a capture is taken, loaded only once at the ScreenLoad startup", DefaultValue="default")]
		...
		[IniProperty("OptimizePNGCommand", Description = "Optional command to execute on a temporary PNG file, the command should overwrite the file and ScreenLoad will read it back. Note: this command is also executed when uploading PNG's!", DefaultValue = "")]
		public string OptimizePNGCommand { get; set; }

        [IniProperty("OptimizePNGCommandArguments", Description = "Arguments for the optional command to execute on a PNG, {0} is replaced by the temp-filename from ScreenLoad. Note: Temp-file is deleted afterwards by ScreenLoad.", DefaultValue = "\"{0}\"")]
		public string OptimizePNGCommandArguments { get; set; }

		[IniProperty("LastSaveWithVersion", Description = "Version of ScreenLoad which created this .ini")]
			...
			switch(property) {
				...
				case "OutputFileAsFullpath":
					if (IniConfig.IsPortable) {
						return Path.Combine(Application.StartupPath, @"..\..\Documents\Pictures\ScreenLoads\dummy.png");
					}
					return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop),"dummy.png");
				case "OutputFilePath":
					if (IniConfig.IsPortable) {
						string pafOutputFilePath = Path.Combine(Application.StartupPath, @"..\..\Documents\Pictures\ScreenLoads");
