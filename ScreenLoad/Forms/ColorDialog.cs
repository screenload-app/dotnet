...
namespace ScreenLoad
{
    ...
    public partial class ColorDialog : BaseForm
    {
        ...
        public ColorDialog()
        {
            ...
            CreateColorPalette(12, 12, 14, 14);
            CreateLastUsedColorButtonRow(12, 206, 14, 14);
        ...
        private void CreateColorButtonColumn(int red, int green, int blue, int x, int y, int w, int h, int shades)
        {
            ...
            {
                _colorButtons.Add(CreateColorButton(red * i / shadedColorsNum, green * i / shadedColorsNum,
                    blue * i / shadedColorsNum, x, y + i * (h + 1), w, h));

                if (i > 0)
                    _colorButtons.Add(CreateColorButton(red + (255 - red) * i / shadedColorsNum,
                        green + (255 - green) * i / shadedColorsNum, blue + (255 - blue) * i / shadedColorsNum, x,
                        y + (i + shadedColorsNum) * (h + 1), w, h));
            }
        }
        ...
        private void TextBoxRgbTextChanged(object sender, EventArgs e)
        {
            ...
            TextBox textBox = (TextBox) sender;
            PreviewColor(
                Color.FromArgb(GetColorPartIntFromString(textBoxAlpha.Text), GetColorPartIntFromString(textBoxRed.Text),
                    GetColorPartIntFromString(textBoxGreen.Text), GetColorPartIntFromString(textBoxBlue.Text)),
                textBox);
        }
