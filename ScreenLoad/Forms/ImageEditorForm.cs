...
using ScreenLoad.Drawing.Fields.Binding;
...
namespace ScreenLoad {
	...
	public partial class ImageEditorForm : BaseForm, IImageEditor {
		...
		private ScreenLoadToolStripButton[] _toolbarButtons;
		...
		/**
		 * Interfaces for plugins, see ScreenLoadInterface for more details!
		 */
		...
		private void UncheckAllToolButtons() {
			if (_toolbarButtons != null) {
				foreach (ScreenLoadToolStripButton butt in _toolbarButtons) {
					butt.Checked = false;
				}
			}
		}
		...
		private void SaveElementsToolStripMenuItemClick(object sender, EventArgs e) {
			...
			{
				Filter = "ScreenLoad templates (*.gst)|*.gst",
